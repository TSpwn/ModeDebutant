...
> 🇬🇧 **English summary** — Beginner-friendly plugin for N.I.N.A. (French UI). Two imaging-tab
> panels: **Simplified Polar Alignment** (turns TPPA measurements into plain physical
> instructions — which knob, which direction, big arrows, color scale) and a **Simplified
> Sequencer** (search a target by name, GoTo, set 3 numbers, press GO — it builds and runs a
> real advanced sequence). Extras: tonight's best-target suggestions, hourly cloud forecast,
> phone notifications via ntfy, automatic darks at the end of the run, and an HTML night report
> with an HFR curve. The UI is French-only for now; the code and this technical doc are easy to
> follow if you want to contribute.
